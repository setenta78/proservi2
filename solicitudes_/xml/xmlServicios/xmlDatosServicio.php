<?
	...
	$objServicios->buscaSimccarAsignadas($unidad, $correlativo, &$mediosVigilancia);
   		...
   		if ($mediosVigilancia != ""){
   			...
   			for ($i=0; $i<$cantidadMediosVigilancia; $i++){
   				if ($mediosVigilancia[$i]->getVehiculo()->getCodigoVehiculo() == "") {
   				 ...
   				 if ($mediosVigilancia[$i]->getAnimal()->getCodigoCaballo() == "") {
   					$codigoCaballo 	= 0;
   					$nombreAnimal  = "";
   					$tipoAnimal		= "SIN ANIMAL";
   				} else {
   					$codigoCaballo 	= $mediosVigilancia[$i]->getAnimal()->getCodigoCaballo();
   					$nombreAnimal 	= $mediosVigilancia[$i]->getAnimal()->getNombreCaballo();
   					$tipoAnimal 		= $mediosVigilancia[$i]->getTipoAnimal()->getDescripcion();
   				}
   				
   				//if ($mediosVigilancia[$i]->getSimccar()->getCaptura() == "") {
   				//	$codigoSimccar 	= 0;
   				//	$serieSimccar  = "SIN SIMCCAR";
   				//} else {
   				//	$codigoSimccar 	= $mediosVigilancia[$i]->getSimccar()->getCaptura();
   				//	$serieSimccar 	= $mediosVigilancia[$i]->getSimccar()->getMes();
   				//}
   				...
   				echo "<codigoCaballo>".$codigoCaballo."</codigoCaballo>";
   				echo "<nombreCaballo>".$nombreAnimal."</nombreCaballo>";
   				echo "<tipoAnimal>".$tipoAnimal."</tipoAnimal>";
   				
   				//echo "<codigoSimccar>".$codigoSimccar."</codigoSimccar>";
   				//echo "<serieSimccar>".$serieSimccar."</serieSimccar>";
	   			...
	   			for ($j=0; $j<$mediosVigilancia[$i]->getCantidadDeFuncionarios(); $j++){
   					...
   					echo "<simccars>";
	   				for ($k=0; $k<$mediosVigilancia[$i]->getFuncionarios($j)->getCantidadSimccars(); $k++){
			   			echo "<simccar>";
			   			echo "<codigoSimccar>".$mediosVigilancia[$i]->getFuncionarios($j)->getSimccars($k)->getCodigoSimccar()."</codigoSimccar>";
			   			echo "<numeroSerie>".$mediosVigilancia[$i]->getFuncionarios($j)->getSimccars($k)->getSerieSimccar()."</numeroSerie>";
	   					echo "</simccar>";	
	   				}
	   				echo "</simccars>";
