...
Class dbUnidad extends Conexion{	
	...
	function PrimerArbolUnidades($codPadre,$perfil,$unidades){
		...
		elseif($codPadre==9650 || $codPadre==10580 || $codPadre==10540){
			//Prefectura Transito y Carreteras Y CENCO
	...
	function ArbolUnidades($codPadre,$jerarquia,$perfil,$unidades){
		...
		if($jerarquia==1){
			//Zona
			$sql = "SELECT PREFECTURA_CODIGO AS UNI_CODIGO, PREFECTURA_DESCRIPCION AS UNI_DESCRIPCION, ZONA_CODIGO AS UNI_PADRE, if((PREFECTURA_TIPOUNIDAD = 30 OR PREFECTURA_TIPOUNIDAD = 120 OR PREFECTURA_TIPOUNIDAD = 150 OR PREFECTURA_TIPOUNIDAD = 200),PREFECTURA_TIPOUNIDAD,IF(UNI_TIPOUNIDAD=50,50,0)) AS UNI_TIPO, PREFECTURA_ESPECIALIDAD AS UNI_ESPECIALIDAD 
				FROM VISTA_ARBOL_UNIDADES_NACIONAL
				WHERE ZONA_CODIGO = ".$codPadre." ".$filtro."
				GROUP BY PREFECTURA_CODIGO, PREFECTURA_DESCRIPCION
				ORDER BY ZONA_ORDEN";
