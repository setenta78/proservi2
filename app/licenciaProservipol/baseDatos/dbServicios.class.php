...
{			
    ...
    //Se agrego el join de la tabla UNIDAD a UNIDAD_CUADRANTE
    //Se agrego el campo UNI_DESCRIPCION para asociar los cuadrantes a la descripcion de la unidad en los Destinos
	function buscaCuadrantesAsignados($unidad, $correlativo, $mediosVigilancia){
		...
		while($myrow = mysql_fetch_array($result)){
			...
			$cuadrante->setDescripcion($myrow["CUA_DESCRIPCION"]);
  	        $cuadrante->setDescUni($myrow["UNI_DESCRIPCION"]); //Campo añadido el 22-04-2015
    	...
    	//NUEVA FUNCION AÑADIDA EL 22-04-2015
        //FUNCION QUE PERMITE OBTENER LAS UNIDADES ASIGNADAS A UN SERVICIO
		...
		$sql = "SELECT DISTINCT 
				...
				FROM
				  UNIDAD_SERVICIO
				  INNER JOIN FUNCIONARIO_SERVICIO ON (UNIDAD_SERVICIO.UNI_CODIGO = FUNCIONARIO_SERVICIO.UNI_CODIGO)
		  ...
		  INNER JOIN UNIDAD ON (UNIDAD_SERVICIO.UNIDAD_SERVICIO = UNIDAD.UNI_CODIGO)
				WHERE
				  UNIDAD_SERVICIO.UNI_CODIGO = ".$unidad." AND 
				  FUNCIONARIO_SERVICIO.CORRELATIVO_SERVICIO = " . $correlativo ."
    ...
    //NUEVA FUNCION AÑADIDA EL 22-04-2015
    //FUNCION QUE PERMITE INSERTAR LOS DESTINOS ASIGNADOS A UN SERVICIO
	 function insertDestinoServicio($servicio){
 ...
 //FIN NUEVA FUNCION
	...
	function insertNuevoServicio($servicio){
		...
		if ($result == 1){
            ...
            $resultInsertDestinosAsignados	    = $this->insertDestinoServicio($servicio); //Llamada a funcion agregada el 22-04-2015
		}
	...
	function updateServicio($servicio){
		...
		if ($result == 1){
			...
			$resultBorrarFuncionariosAsignados		= $this->borrarFuncionariosServicio($servicio);
  	        $resultBorrarDestinosAsignados		    = $this->borrarDestinoServicio($servicio); //Llamada a funcion agregada el 22-04-2015
			...
			if ($resultBorrarFuncionarioVehiculo == 1)	 $resultInsertFuncionarioVehiculo	= $this->insertFuncionarioVehiculo($servicio);
  	        if ($resultBorrarDestinosAsignados	 == 1)	 $resultInsertDestinosAsignados	    = $this->insertDestinoServicio($servicio); //Llamada a funcion agregada el 22-04-2015
		}
	...
	//NUEVA FUNCION AÑADIDA EL 22-04-2015
    //ESTA FUNCION TIENE POR FINALIDAD ELMINAR LOS DESTINOS ASIGNADOS
	function borrarDestinoServicio($servicio){
	...
	function deleteServicio($servicio){
		...
		$resultBorrarVehiculosAsignados			= $this->borrarVehiculosServicio($servicio);
		$resultBorrarFuncionariosAsignados		= $this->borrarFuncionariosServicio($servicio);
        $resultBorrarDestinosAsignados		    = $this->borrarDestinoServicio($servicio); //Llamada a funcion agregada el 22-4-2015
		...
		function listaServiciosAcumuladoNuevo($unidad, $tipoUnidad, $tipoUnidadPadre, $tipoServicio, $fecha1, $inicio, $serviciosIngresados){	
			...
			$sql = "SELECT 
					...
					FROM
					  ...
					  AND (SERVICIO.CORRELATIVO_SERVICIO = FUNCIONARIO_SERVICIO.CORRELATIVO_SERVICIO)
					  INNER JOIN TIPO_SERVICIO ON (SERVICIO.TSERV_CODIGO = TIPO_SERVICIO.TSERV_CODIGO)
					  INNER JOIN VISTA_UNIDADES_2 ON (SERVICIO.UNI_CODIGO = VISTA_UNIDADES_2.DESTACAMENTO_CODIGO)
					  LEFT OUTER JOIN FUNCIONARIO_VEHICULO ON (FUNCIONARIO_SERVICIO.UNI_CODIGO = FUNCIONARIO_VEHICULO.FUN_UNI_CODIGO)
					  AND (FUNCIONARIO_SERVICIO.CORRELATIVO_SERVICIO = FUNCIONARIO_VEHICULO.FUN_CORRELATIVO_SERVICIO)
					  AND (FUNCIONARIO_SERVICIO.FUN_CODIGO = FUNCIONARIO_VEHICULO.FUN_CODIGO)
					WHERE
				...
				$sql = "SELECT 
					  VISTA_UNIDADES.".$tipoUnidadPadre."_CODIGO AS UNI_CODIGO_PADRE ,
					  VISTA_UNIDADES.".$tipoUnidad."_CODIGO AS UNI_CODIGO ,
					  VISTA_UNIDADES.".$tipoUnidad."_DESCRIPCION AS UNI_DESCRIPCION,
					  SERVICIO.TSERV_CODIGO AS TSERV_CODIGO,
					  UCASE(TIPO_SERVICIO.TSERV_DESCRIPCION) AS TSERV_DESCRIPCION,
					...
					FROM
					  ...
					  AND (SERVICIO.CORRELATIVO_SERVICIO = FUNCIONARIO_SERVICIO.CORRELATIVO_SERVICIO)
					  INNER JOIN TIPO_SERVICIO ON (SERVICIO.TSERV_CODIGO = TIPO_SERVICIO.TSERV_CODIGO)
					  INNER JOIN VISTA_UNIDADES ON (SERVICIO.UNI_CODIGO = VISTA_UNIDADES.DESTACAMENTO_CODIGO)
					  LEFT OUTER JOIN FUNCIONARIO_VEHICULO ON (FUNCIONARIO_SERVICIO.UNI_CODIGO = FUNCIONARIO_VEHICULO.FUN_UNI_CODIGO)
					  AND (FUNCIONARIO_SERVICIO.CORRELATIVO_SERVICIO = FUNCIONARIO_VEHICULO.FUN_CORRELATIVO_SERVICIO)
					  AND (FUNCIONARIO_SERVICIO.FUN_CODIGO = FUNCIONARIO_VEHICULO.FUN_CODIGO)
					WHERE
					  SERVICIO.FECHA = '".$fecha1."'
					GROUP BY
					  VISTA_UNIDADES.".$tipoUnidadPadre."_CODIGO,
					  VISTA_UNIDADES.".$tipoUnidad."_CODIGO,
					  VISTA_UNIDADES.".$tipoUnidad."_DESCRIPCION,
					  SERVICIO.TSERV_CODIGO,
					  UCASE(TIPO_SERVICIO.TSERV_DESCRIPCION),
					  SERVICIO.CORRELATIVO_SERVICIO
					HAVING UNI_CODIGO_PADRE = ".$unidad." ".$filtroServicio."
...
ORDER BY SERVICIO.FECHA DESC, TIPO_SERVICIO.TSERV_DESCRIPCION ASC";
		...
		function buscaTiposDeServiciosPorFuncionario($unidadServicio, $fecha1, $fecha2, $tipoServicio, $codigoFuncionario, $cantidadColaciones, $cantidadOtrosServicios){	
