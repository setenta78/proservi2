...
Class dbUsuarios extends Conexion{
	...
	function validaUsuario($login, $password, $aplicacion, $usuario){
		...
		if(mysql_num_rows($result) > 0){
	  	$myrow = mysql_fetch_array($result);
		 	...
		 	$unidad->setTipoUnidad($myrow["UNI_TIPOUNIDAD"]); //Campo añadido el 14-09-2014
		 	$unidad->setContieneHijos($myrow["UNI_CONTIENEHIJOS"]); //Campo añadido el 16-04-2014
		 	$unidad->setUnidadTipo($myrow["TUNI_CODIGO"]); //Campo añadido el 17-06-2019
  ...
  function modificaBitacoraUsuario($userID,$unidad,$hra_inicio,$hra_termino,$perfil,$evento){
	...
	function CrearUsuario($funcionario, $TipoUsuario){
		...
		$sql = "INSERT INTO USUARIO (FUN_CODIGO, UNI_CODIGO, US_LOGIN, US_PASSWORD, TUS_CODIGO, US_FECHACREACION)
						VALUES ('".$funcionario->getCodigoFuncionario()."',".$funcionario->getUnidad()->getCodigoUnidad().",'".$funcionario->getCodigoFuncionario()."','".$password."','".$TipoUsuario."',CURDATE())";
	...
	function modificaUsuario($funcionario, $TipoUsuario){
		
		$sql = "UPDATE USUARIO
				SET TUS_CODIGO = '".$TipoUsuario."'
				WHERE
				USUARIO.FUN_CODIGO = '".$funcionario->getCodigoFuncionario()."'";
