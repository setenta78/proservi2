<?
  ...
  header("Content-Disposition:attachment;  filename=\"INGRESO_REUNIONES_RRCC.XLS\";");
  ...
  echo "<tr>
  <th colspan=4>INFORME UNIDADES SIN INGRESO DE REUNIONES MODULO RELACIONES COMUNITARIAS</th>
  </tr>";

  echo "<tr>
  <th colspan=4>".$meses[$mes-1]." DE ".$anno."</th>
  </tr>";
  ...
  <th>UNIDAD</th>
     ...
     for($j = 0, $size2 = sizeof($arregloUnidades[0]); $j < $size2; $j=$j+1)
     {
        if($j == 0)
        {
          $consulta = $arregloUnidades[0][$j];
        }

        else
        {
          $consulta .= " OR UNIDAD.UNI_CODIGO=".$arregloUnidades[0][$j];
        }
     }
