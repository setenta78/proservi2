...
while (($datos = fgetcsv($handle, 1000, ";")) !== FALSE) {
    ...
    */
    ...
    //$sqlDuplicidad = sprintf("SELECT VC_COD_EQUIPO_SAP FROM VIDEOCAMARA WHERE VC_COD_EQUIPO_SAP = '%s'", mysql_real_escape_string($codEquipoSap));
...
foreach ($updateData as $data) {
    ...
    // Construye la consulta con sprintf
    /*   $updateQuery = sprintf(
        "UPDATE VIDEOCAMARA 
        SET MVC_CODIGO = '%d', MODVC_CODIGO = '%d', PREC_CODIGO = '%d', VC_COD_ACTIVO_SAP = '%s', VC_NRO_SERIE = '%s' 
        WHERE VC_COD_EQUIPO_SAP = '%s'",
        $marca,
        $modelo,
        $procedencia,
        $codActivoSap,
...
echo "Filas omitidas por serie vacia: $contadorNoInsertados<br>";
