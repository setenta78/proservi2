<?
  ...
  $animal->setSeccion($seccion); //Instancia agregada a 29-04-2015
	
	$objDBAnimales = new dbAnimal;
	$resultado = $objDBAnimales->updateAnimal($animal);
	
	if ($fechaNuevoEstado != ""){
		$fechaPaso 			= explode("-",$fechaNuevoEstado);
   	$fechaIngresar  = $fechaPaso[2] . "-" . $fechaPaso[1] . "-" . $fechaPaso[0];
		$resultado = $objDBAnimales->updateEstadoAnimal($animal, $fechaIngresar);
