...
function listaServiciosPorFuncionario(){
	...
	var func      = document.getElementById("selFiltroBusqueda").value;
	var f1 			  = document.getElementById("textFechaDesde").value;
	var f2 			  = document.getElementById("textFechaHasta").value;
	...
	//cabecera += "<td id='nombreColumna' width='55%' align='center'>SERVICIO</td>";
	cabecera += "<td id='nombreColumna' width='55%' align='center'>SERVICIO <a href=\"./xml/xmlServicios/excelListaServiciosPorFuncionario.php?func="+func+"&f1="+f1+"&f2="+f2+"\"\"><img src='img/microsoft-excel.png' width='30' height='32' border='0' align='middle' alt='Descargar Excel'/></a></td>";
	...
	var codigoServicio 	  = document.getElementById("selFiltroServicio").value;
	
	if (codigoServicio != 0) codigoServicio = codigoServicio.substring(1,codigoServicio.length);
	...
	objHttpXMLServicios.send(encodeURI("codigoFuncionario="+codigoFuncionario+"&fecha1="+fecha1+"&fecha2="+fecha2+"&codigoServicio="+codigoServicio));  
		...
		{       
			...
			if (objHttpXMLServicios.responseText != "VACIO"){
				...
				for(i=0;i<xml.getElementsByTagName('servicio').length;i++){
					...
					unidad	 			= xml.getElementsByTagName('codUnidad')[i].text;
...
function listaServiciosPorVehiculo(){
	...
	var veh       = document.getElementById("selFiltroBusqueda").value;
	var f1 			  = document.getElementById("textFechaDesde").value;
	var f2 			  = document.getElementById("textFechaHasta").value;
	...
	//cabecera += "<td id='nombreColumna' width='50%' align='center'>SERVICIO</td>";
	cabecera += "<td id='nombreColumna' width='50%' align='center'>SERVICIO <a href=\"./xml/xmlServicios/excelListaServiciosPorVehiculo.php?veh="+veh+"&f1="+f1+"&f2="+f2+"\"\"><img src='img/microsoft-excel.png' width='30' height='32' border='0' align='middle' alt='Descargar Excel'/></a></td>";
	...
	objHttpXMLServicios.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	objHttpXMLServicios.send(encodeURI("codigoVehiculo="+codigoVehiculo+"&fecha1="+fecha1+"&fecha2="+fecha2+"&codigoServicio="+codigoServicio));  
		...
		{       
			...
			if (objHttpXMLServicios.responseText != "VACIO"){
				...
				for(i=0;i<xml.getElementsByTagName('servicio').length;i++){
					...
					tipoServicio	 	= xml.getElementsByTagName('desServicio')[i].text;
					tipoExtraordinario	= xml.getElementsByTagName('desServicioExtraordinario')[i].text;
					kmInicial		 	= xml.getElementsByTagName('kmInicial')[i].text;
					...
					listadoServicios += "<td width='50%'><div id='valorColumna'>"+tipoServicio.toUpperCase()+"</div></td>";
