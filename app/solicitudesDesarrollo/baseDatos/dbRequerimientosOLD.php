...
{		
	...
	function listaRequerimiento($Unidad, $nombreBucar, $escalafon, $grado, $NombreCampo, $TipoOrden, $funcionarios){
		...
		$FechaHoy = date("Y-m-d");
              ...
              FROM
               `SOLICITUD`
                ...
                INNER JOIN `TIPO_MOVIMIENTO` ON (`MOVIMIENTO`.`TMOV_CODIGO` = `TIPO_MOVIMIENTO`.`TMOV_CODIGO`)
                 ...
                 INNER JOIN UNIDAD ON (UNIDAD.UNI_CODIGO = SOLICITUD.UNI_CODIGO)
            INNER JOIN GRADO ON (FUNCIONARIO.ESC_CODIGO = GRADO.ESC_CODIGO)
             AND (FUNCIONARIO.GRA_CODIGO = GRADO.GRA_CODIGO)
             WHERE
             TMOV_DESCRIPCION <> 'CIERRE: RESUELTO FAVORABLEMENTE' AND TMOV_DESCRIPCION <> 'CIERRE: RESUELTO DESFAVORABLEMENTE' 
                AND TMOV_DESCRIPCION <> 'CIERRE: INADMISIBLE'  AND
              `SOLICITUD`.`UNI_CODIGO`=".$Unidad." AND FECHA_TERMINO IS NULL 
              AND VISIBLE=1 
         ...
         $sql .=" ORDER BY SOLICITUD.SOL_FECHA DESC";
          ...
          $dioscar->setNombreUnidad(STRTOUPPER($myrow["UNI_DESCRIPCION"]));
		...
		function listaRequerimientoCerradas($Unidad, $nombreBucar, $escalafon, $grado, $NombreCampo, $TipoOrden, $funcionarios){
		...
		$FechaHoy = date("Y-m-d");
            ...
            INNER JOIN GRADO ON (FUNCIONARIO.ESC_CODIGO = GRADO.ESC_CODIGO)
             AND (FUNCIONARIO.GRA_CODIGO = GRADO.GRA_CODIGO)
             WHERE
              `SOLICITUD`.`UNI_CODIGO`=".$Unidad." AND
              TMOV_DESCRIPCION <> 'PENDIENTE' AND TMOV_DESCRIPCION <> 'OFICIALIZADO' AND 
              TMOV_DESCRIPCION <> 'EN TRAMITE: EVALUACION DE ANTECEDENTES' AND TMOV_DESCRIPCION <> 'EN TRAMITE: REQUIERE ANTECEDENTES FALTANTES' 
                AND TMOV_DESCRIPCION <> 'EN TRAMITE: ENVIA ANTECEDENTES FALTANTES' AND TMOV_DESCRIPCION <> 'EN TRAMITE: DERIVADO A INFORMATICA' 
                AND TMOV_DESCRIPCION <> 'EN TRAMITE: DERIVADO A O.P.U.'
              AND FECHA_TERMINO IS NULL 
              AND VISIBLE=1 AND SOL_FECHA BETWEEN '20210101' AND '20491231'
              ORDER BY
          ...
          $dioscar->setCorrelativoMov(STRTOUPPER($myrow["MOV_CODIGO"]));
				...
				function listaAntecedentesFaltantes($Unidad, $nombreBucar, $escalafon, $grado, $NombreCampo, $TipoOrden, $funcionarios){
		...
		$FechaHoy = date("Y-m-d");
            ...
            INNER JOIN GRADO ON (FUNCIONARIO.ESC_CODIGO = GRADO.ESC_CODIGO)
             AND (FUNCIONARIO.GRA_CODIGO = GRADO.GRA_CODIGO)
             WHERE
              TMOV_DESCRIPCION = 'EN TRAMITE: REQUIERE ANTECEDENTES FALTANTES' 
              AND FECHA_TERMINO IS NULL 
              AND VISIBLE=1 AND `SOLICITUD`.`SOL_FECHA` BETWEEN '20210801' AND '20491231'
              ORDER BY
              SOLICITUD.SOL_FECHA DESC
              "; 
		        		    		       											
				//echo $sql;
				//MONTH(`SOLICITUD`.`SOL_FECHA`) = MONTH(CURDATE())
				$i=0;
				$result = $this->execstmt($this->Conecta(),$sql);
				mysql_close();
				while($myrow = mysql_fetch_array($result)){
		...
		$FechaHoy = date("Y-m-d");
            ...
            INNER JOIN GRADO ON (FUNCIONARIO.ESC_CODIGO = GRADO.ESC_CODIGO)
             AND (FUNCIONARIO.GRA_CODIGO = GRADO.GRA_CODIGO)
                WHERE
                (TMOV_DESCRIPCION = 'OFICIALIZADO' OR TMOV_DESCRIPCION = 'EN TRAMITE: ENVIA ANTECEDENTES FALTANTES') AND  SOLICITUD.SOL_FECHA BETWEEN '20210801' AND '20491231'
                AND FECHA_TERMINO IS NULL
         
           "; 
         //AND MOVIMIENTO.SDEPTO_CODIGO NOT IN(30) AND TMOV_DESCRIPCION <> 'EN TRAMITE: REQUIERE ANTECEDENTES FALTANTES' AND MONTH(`SOLICITUD`.`SOL_FECHA`) = MONTH(CURDATE())  
         //TMOV_DESCRIPCION <> 'CIERRE: RESUELTO FAVORABLEMENTE' AND TMOV_DESCRIPCION <> 'CIERRE: RESUELTO DESFAVORABLEMENTE' 
         ...
         //AND MOVIMIENTO.SDEPTO_CODIGO NOT IN(30) AND TMOV_DESCRIPCION <> 'EN TRAMITE: REQUIERE ANTECEDENTES FALTANTES' AND
        if ($nombreBucar != ""){
			$sql .= " AND (PROBLEMA.PROB_DESCRIPCION LIKE '".$nombreBucar."' OR SUBPROBLEMA.SUBP_DESCRIPCION LIKE '%".$nombreBucar."%') ";        
			}else{
			$sql .= ""; 
         ...
         $sql .=" ORDER BY SOLICITUD.SOL_FECHA DESC";
          ...
          $dioscar->setImplicado(STRTOUPPER($myrow["DATO_OPER"]));
          $dioscar->setCorrelativoMov(STRTOUPPER($myrow["MOV_CODIGO"]));
          $dioscar->setMovimientoCodigo(STRTOUPPER($myrow["TMOV_CODIGO"]));
          $dioscar->setSeccionDepto(STRTOUPPER($myrow["SDEPTO_CODIGO"]));
	...
	function listaTotalMesAnterior($Unidad, $nombreBucar, $escalafon, $grado, $NombreCampo, $TipoOrden, $funcionarios){
		...
		$year = date("Y");
            ...
            INNER JOIN GRADO ON (FUNCIONARIO.ESC_CODIGO = GRADO.ESC_CODIGO)
             AND (FUNCIONARIO.GRA_CODIGO = GRADO.GRA_CODIGO)
                ...
                AND TMOV_DESCRIPCION <> 'CIERRE: INADMISIBLE' AND TMOV_DESCRIPCION <> 'EN TRAMITE: DERIVADO A INFORMATICA' AND TMOV_DESCRIPCION <> 'EN TRAMITE: DERIVADO A O.P.U.'
                AND MOVIMIENTO.SDEPTO_CODIGO NOT IN(30) AND MONTH(`SOLICITUD`.`SOL_FECHA`) = MONTH(DATE_ADD(CURDATE(),INTERVAL -1 MONTH)) AND YEAR(SOL_FECHA)='".$year."'
                AND FECHA_TERMINO IS NULL
		...
		$FechaHoy = date("Y-m-d");
            ...
            INNER JOIN GRADO ON (FUNCIONARIO.ESC_CODIGO = GRADO.ESC_CODIGO)
             ...
             TIPO_MOVIMIENTO.TMOV_CODIGO IN(70,80)
              AND SOLICITUD.SOL_FECHA BETWEEN '20210101' AND '20491231'
		...
		$FechaHoy = date("Y-m-d");
          ...
          SOLICITUD.SOL_FECHA DESC
           "; 
             //WHERE
              //`solicitud`.`UNI_CODIGO`=".$Unidad."
		        //  AND FECHA_TERMINO IS NULL AND MONTH(`SOLICITUD`.`SOL_FECHA`) = MONTH(CURDATE())		    		       											
				//echo $sql;
		...
		//$usuario1="010907Z";
            ...
            INNER JOIN GRADO ON (FUNCIONARIO.ESC_CODIGO = GRADO.ESC_CODIGO)
             AND (FUNCIONARIO.GRA_CODIGO = GRADO.GRA_CODIGO)
                ...
                AND SOLICITUD.SOL_FECHA BETWEEN '20210101' AND '20491231' AND MOVIMIENTO.FECHA_TERMINO IS NULL
         ...
         SOLICITUD.SOL_CODIGO";
          ...
          $dioscar->setCorrelativoMov(STRTOUPPER($myrow["MOV_CODIGO"]));
					...
					$dioscar->setcodUnidad($myrow["UNI_CODIGO"]);
		...
		//$usuario1="010907Z";
               ...
               MOVIMIENTO.FUNCIONARIO_IMPLICADO,
               MOVIMIENTO.FUNCIONARIO_DERIBA,
               CONCAT_WS(' ',  `TIPO_MOVIMIENTO`.`TMOV_DESCRIPCION`,'POR INFORMATICA POR:',GRADO.GRA_DESCRIPCION, FUNCIONARIO.FUN_APELLIDOPATERNO, FUNCIONARIO.FUN_APELLIDOMATERNO, FUNCIONARIO.FUN_NOMBRE) AS DATO_OPER,
               CONCAT_WS(' ', UCASE(`SOLICITUD`.VALOR_IDENTI1), UCASE(`SOLICITUD`.VALOR_IDENTI2)) AS IDENTIFICADORES,
               DATEDIFF(NOW(),FECHA) AS DIF_DIAS,
               MOVIMIENTO.MOV_CODIGO
              FROM
            ...
            INNER JOIN GRADO ON (FUNCIONARIO.ESC_CODIGO = GRADO.ESC_CODIGO)
             AND (FUNCIONARIO.GRA_CODIGO = GRADO.GRA_CODIGO)
                ...
                AND FECHA_TERMINO IS NULL AND SOL_FECHA BETWEEN '20210101' AND '20491231'
                 ORDER BY
            SOLICITUD.SOL_FECHA DESC,
          SOLICITUD.UNI_CODIGO
           "; 
		...
		//$usuario1="010907Z";
               ...
               MOVIMIENTO.FUNCIONARIO_IMPLICADO,
               MOVIMIENTO.FUNCIONARIO_DERIBA,
               CONCAT_WS(' ',  `TIPO_MOVIMIENTO`.`TMOV_DESCRIPCION`,'POR O.P.U. POR:',GRADO.GRA_DESCRIPCION, FUNCIONARIO.FUN_APELLIDOPATERNO, FUNCIONARIO.FUN_APELLIDOMATERNO, FUNCIONARIO.FUN_NOMBRE) AS DATO_OPER,
               CONCAT_WS(' ', UCASE(`SOLICITUD`.VALOR_IDENTI1), UCASE(`SOLICITUD`.VALOR_IDENTI2)) AS IDENTIFICADORES,
               DATEDIFF(NOW(),FECHA) AS DIF_DIAS,
               MOVIMIENTO.MOV_CODIGO
              FROM
          ...
          `SOLICITUD`
            ...
            CURDATE()>=DATE_ADD(FECHA_INICIO,INTERVAL 24 HOUR) AND FECHA_TERMINO IS NULL
            AND FECHA_TERMINO IS NULL"; 
          ...
          $dioscar->setSubproblema($myrow["SUBP_DESCRIPCION"]);
			...
			//       (UNI_CODIGO,FUN_CODIGO,REQ_TIPO,REQ_MODULO,REQ_PROBLEMA,REQ_OBS,FECHA_INICIO,FECHA_TERMINO,ESTADO) VALUES
			// 	    (".$vehiculo->getUnidad().",  
      //      '".$vehiculo->getFuncionario()."',  
      //      ".$vehiculo->getTipoRequerimiento().",  
      //       ".$vehiculo->getModulo().",
      //       ".$vehiculo->getProblema().",
      //      '".$vehiculo->getObservacion()."',
      //      '".$vehiculo->getFechaInicio()."',
      //      '".$vehiculo->getFechaTermino()."',
      //      ".$vehiculo->getEstado().")";
...
AND MOVIMIENTO.FECHA_TERMINO IS NULL";
      ...
      $visible =1;
			...
			$sql = "INSERT INTO MOVIMIENTO (SOL_CODIGO, TMOV_CODIGO, TEXTO, FUNCIONARIO_IMPLICADO, ROL_FUNCIONARIO_IMPLICADO, FECHA,FECHA_INICIO,FUNCIONARIO_DERIBA,SDEPTO_CODIGO,ARCHIVO,VISIBLE)
					VALUES (".$vehiculo->getCodigoSolicitud().",".$vehiculo->getTipoMovimiento().",'".$vehiculo->getTextoMovimiento()."','".$vehiculo->getFuncionario()."',".$usuario.",NOW(),'".$fechaSolicitud."','".$unidadAgregadoGuardar."','".$secciones."','".$vehiculo->getArchivoSolicitud()."',".$visible.")";
      ...
      //$implicado="995762T";
			...
			$sql = "INSERT INTO MOVIMIENTO (SOL_CODIGO, TMOV_CODIGO, TEXTO, FUNCIONARIO_IMPLICADO, ROL_FUNCIONARIO_IMPLICADO,FECHA,FECHA_INICIO,FUNCIONARIO_DERIBA,SDEPTO_CODIGO,ARCHIVO,VISIBLE)
					VALUES (".$vehiculo->getCodigoSolicitud().",".$vehiculo->getCodigoTipoMovimiento().",'".$vehiculo->getTextoMovimiento()."','".$vehiculo->getUsuarioImplicado()."',".$tipoUsuario.",NOW(),'".$vehiculo->getFechaInicio()."','".$funcionarioDerivadoGuardar."',".$vehiculo->getSecciones().",'".$vehiculo->getArchivoSolicitud()."',".$visible.")";
	    ...
	    $sql = "SELECT 
              ...
              VISTA_ARBOL_UNIDADES_NACIONAL.UNI_CODIGO AS UNI_CODIGO,
          ...
          USUARIO
           ...
           WHERE
         VISTA_ARBOL_UNIDADES_NACIONAL.UNI_CODIGO=".$unidad." AND USUARIO.US_LOGIN='".$usuario."'
        ORDER BY 
         ...
         VISTA_ARBOL_UNIDADES_NACIONAL.UNI_DESCRIPCION";
	    	 ...
	    	 //echo $sql;
			  ...
			  $vista = new vista;
				...
				$vista->setZona($myrow["ZONA"]);
				$vista->setPrefectura($myrow["PREFECTURA"]);
				$vista->setComisaria($myrow["COMISARIA"]);
				...
				function datoUsuarioSolicitud($unidad,$solicitud,$usuario,$vista){
	    ...
	    $sql = "SELECT 
          ...
          USUARIO
            ...
            INNER JOIN TIPO_USUARIO ON (USUARIO.TUS_CODIGO = TIPO_USUARIO.TUS_CODIGO)
            INNER JOIN VISTA_ARBOL_UNIDADES_NACIONAL ON (USUARIO.UNI_CODIGO = VISTA_ARBOL_UNIDADES_NACIONAL.UNI_CODIGO)
            INNER JOIN SOLICITUD ON (VISTA_ARBOL_UNIDADES_NACIONAL.UNI_CODIGO = SOLICITUD.UNI_CODIGO)
            INNER JOIN MOVIMIENTO ON (SOLICITUD.SOL_CODIGO = MOVIMIENTO.SOL_CODIGO)
            INNER JOIN PROBLEMA ON (SOLICITUD.PROB_CODIGO = PROBLEMA.PROB_CODIGO)
            INNER JOIN SUBPROBLEMA ON (SOLICITUD.SUBP_CODIGO = SUBPROBLEMA.SUBP_CODIGO)
            INNER JOIN TIPO_MOVIMIENTO ON (MOVIMIENTO.TMOV_CODIGO = TIPO_MOVIMIENTO.TMOV_CODIGO)
           WHERE
         VISTA_ARBOL_UNIDADES_NACIONAL.UNI_CODIGO=".$unidad." AND SOLICITUD.SOL_CODIGO=".$solicitud." AND USUARIO.US_LOGIN='".$usuario."'
         AND FECHA_TERMINO IS NULL
         ORDER BY 
         VISTA_ARBOL_UNIDADES_NACIONAL.ZONA_DESCRIPCION,
         VISTA_ARBOL_UNIDADES_NACIONAL.PREFECTURA_DESCRIPCION,
  ...
  VISTA_ARBOL_UNIDADES_NACIONAL.UNI_DESCRIPCION";
	...
	function datoSolicitud($unidad,$codigo,$solicitud){
	    ...
	    `SOLICITUD`.`SOL_CODIGO`,
          ...
          WHERE
          UNIDAD.UNI_CODIGO=".$unidad." AND SOLICITUD.SOL_CODIGO=".$codigo." AND FECHA_TERMINO IS NULL
          ";
          ...
          $solicitud->setSecciones($myrow["SDEPTO_CODIGO"]);
          $solicitud->setNombreUnidad(STRTOUPPER($myrow["UNI_DESCRIPCION"]));
          $solicitud->setPassUsuario(STRTOUPPER($myrow["US_PASSWORD"]));
