...
function leeServicios(unidad, fecha1, fecha2, servicios){
	...
	var tipoUnidad = document.getElementById("tipoUnidad").value; //VARIABLE AGREGADA EL 22-04-2015
  //  alert(tipoUnidad);
	...
	//alert("Fecha Limite: "+comparaFechaLimite);
	...
	objHttpXMLServicios.onreadystatechange=function()
	{
		...
		{       
			...
			if(tipoUnidad==30 || tipoUnidad==120 || tipoUnidad==130 || tipoUnidad==135){
				//CONTROL AGREGADO EL 22-04-2015
				...
				//FIN CONTROL AGREGADO	
					...
					//alert("servicio n"+(i+1)+" :"+tipoServicio);
					listadoServicios += "<tr id='trNro"+i+"' OnMouseOver=\""+resaltarLinea+"\" OnMouseOut=\""+lineaSinResaltar+"\" class='"+fondoLinea+"' onDblClick=\""+dblClick+"\">";
					listadoServicios += "<td width='5%' align='center'><div id='valorColumna'>"+(i+1)+"</div></td>";
					listadoServicios += "<td width='15%' align='center'><div id='valorColumna'>"+fecha+"</div></td>";
					listadoServicios += "<td width='55%'><div id='valorColumna'>"+tipoServicio.toUpperCase()+"</div></td>";
...
if(tipoUnidad==30 || tipoUnidad==120 || tipoUnidad==130 || tipoUnidad==135)
{
//INICIO DE MODIFICACION 22-04-2015
	...
	for(var i=0;i<mediosDeVigilancia.getElementsByTagName('medioVigilancia').length;i++){
		...
		if (document.getElementById("tipoServicio").value == "O"){
			...
			for (var j=0; j<cuadrantes.getElementsByTagName('destino').length; j++){
				...
				var cuadrante = cuadrantes.getElementsByTagName('descripcionDestino')[j].text;
 ...
 if(tipoUnidad == 30 || tipoUnidad==120 || tipoUnidad==130 || tipoUnidad==135){
  for(var i=0;i<mediosDeVigilancia.getElementsByTagName('medioVigilancia').length;i++){
		...
		for(var j=0;j<destinos.getElementsByTagName('destino').length;j++){
			var codigoDestino 	= destinos.getElementsByTagName('codigoDestino')[j].text;
			var descripcionDestino 	= destinos.getElementsByTagName('descripcionDestino')[j].text;				
			var destinoDesc = descripcionDestino;
			var datosOpcion = new Option(destinoDesc, codigoDestino, "", "");
		...
		for(var j=0;j<cuadrantes.getElementsByTagName('cuadrante').length;j++){
			...
			//var descripcionCuadrante 	= cuadrantes.getElementsByTagName('descripcionCuadrante')[j].text;	
            var descripcionCuadrante 	= cuadrantes.getElementsByTagName('descripcionCuadrante')[j].text;	
            var unidadDescripcion       = cuadrantes.getElementsByTagName('descUni')[j].text;		
			var cuadranteDesc = descripcionCuadrante+" ("+unidadDescripcion+")";
            var codigoCuadrante2=codigoCuadrante+"C";
			//var cuadranteDesc = descripcionCuadrante;
