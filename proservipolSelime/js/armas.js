		...
		{       
			...
			if (objHttpXMLArmas.responseText != "VACIO"){
				...
				for(i=0;i<xml.getElementsByTagName('arma').length;i++){
					...
					var nroLinea = i + 1;
					var dblClick = "javascript:abrirVentana('ARMA ...', '800', '250','fichaArma.php?codigoArma="+codigo+"','"+nroLinea+"','','5','5')";
		...
		{       
			...
			if (objHttpXMLArmas.responseText != "VACIO"){
				...
				for(i=0;i<xml.getElementsByTagName('arma').length;i++){
					...
					var valoresAsignar = "'" + tipo + "','" + marca + "','" + modelo + "','" + estado + "'," + habilitarBotones;   
...
function asignaValores(tipo,marca,modelo,estado, habilitarBotones){
...
function actualizarArma(codigoArma){
	...
	parametros += "codigo="+codigoArma+"&numeroSerie="+numeroSerie;
