function abrirVentanaUsuario(){
	var titulo = "USUARIO (MODIFICA CONTRASEÑA)";
	var ancho  = 700;
	var alto   = 240;
...
function cambiarContrasena(){
	document.getElementById("ventanaCambiaContraseña").style.display = "";
...
function cerrarCambiarContrasena(){
	document.getElementById("ventanaCambiaContraseña").style.display = "none";
...
function obtieneClaveUsuario(codigoFuncionario){
	...
	var clave = xml.getElementsByTagName('claveUsuario')[0].text;
	return clave
}
	...
	objHttpXMLUsuario.onreadystatechange=function()
	{
		...
		{       
				...
				for(i=0;i<xml.getElementsByTagName('resultado').length;i++){
					...
					if (codigo == 1){
						 alert('LA CONTRASEÑA FUE ACTUALIZADA CON EXITO EN LA BASE DE DATOS.	\nLA APLICACION SE CERRARA PARA NUEVA CONTRASEÑA.');
						 //cerrarCambiarContrasena();
						 top.cerrarAplicacion();
					}
					else {
						alert('LA CONTRASEÑA NO PUDO SER ACTUALIZADA EN LA BASE DE DATOS ....		\nCODIGO RECIBIDO : ' + codigo);
						return false;
					}
				}
			}
