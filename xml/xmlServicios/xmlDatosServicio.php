<?
	...
	echo "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>";
  	echo "<root>";
   		...
   		echo "<codUnidadDestino>".$servicio->getDestino()->getCodigoUnidad()."</codUnidadDestino>";
   		echo "<descUnidadDestino>".$servicio->getDestino()->getDescripcionUnidad()."</descUnidadDestino>";
   		echo "</identificacionServicio>";
   		if ($mediosVigilancia != ""){
   			...
   			for ($i=0; $i<$cantidadMediosVigilancia; $i++){
   				...
   				if ($mediosVigilancia[$i]->getAnimal()->getCodigoAnimal() == "") {
   					$codigoAnimal 	= 0;
   					$nombreAnimal  = "";
   					$tipoAnimal		= "SIN ANIMAL";
   				} else {
   					$codigoAnimal 	= $mediosVigilancia[$i]->getAnimal()->getCodigoAnimal();
   					$nombreAnimal 	= $mediosVigilancia[$i]->getAnimal()->getNombreAnimal();
   					$tipoAnimal 		= $mediosVigilancia[$i]->getTipoAnimal()->getDescripcion();
   				}
   				...
   				echo "<codigoAnimal>".$codigoAnimal."</codigoAnimal>";
   				echo "<nombreAnimal>".$nombreAnimal."</nombreAnimal>";
   				echo "<tipoAnimal>".$tipoAnimal."</tipoAnimal>";
	   			...
	   			for ($j=0; $j<$mediosVigilancia[$i]->getCantidadDeFuncionarios(); $j++){
					...
					echo "<camaras>";
	   				for ($k=0; $k<$mediosVigilancia[$i]->getFuncionarios($j)->getCantidadCamaras(); $k++){
			   			echo "<camara>";
			   			echo "<codigoCamara>".$mediosVigilancia[$i]->getFuncionarios($j)->getCamaras($k)->getCodigo()."</codigoCamara>";
			   			echo "<modeloCamara>".$mediosVigilancia[$i]->getFuncionarios($j)->getCamaras($k)->getModelo()."</modeloCamara>";
			   			echo "<numeroSerieCamara>".$mediosVigilancia[$i]->getFuncionarios($j)->getCamaras($k)->getNumeroSerie()."</numeroSerieCamara>";
	   					echo "</camara>";
	   				}
   					echo "</camaras>";
	   			...
	   			for ($j=0; $j<$mediosVigilancia[$i]->getCantidadDeCuadrantes(); $j++){
	   				...
	   				echo "<descUni>".$mediosVigilancia[$i]->getCuadrantes($j)->getDescUni()."</descUni>";
	   			...
	   			for ($j=0; $j<$mediosVigilancia[$i]->getCantidadDeUnidades(); $j++){
	   				echo "<destino>";
	   				echo "<codigoDestino>".$mediosVigilancia[$i]->getUnidades($j)->getCodigoUnidad()."</codigoDestino>";
	   				echo "<descripcionDestino>".$mediosVigilancia[$i]->getUnidades($j)->getDescripcionUnidad()."</descripcionDestino>";
   					echo "</destino>";
	   			}
	   			echo "</destinos>";
   				echo "</medioVigilancia>";
