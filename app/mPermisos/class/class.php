...
class Trabajo
{
	...
	//print_r($_POST);
        $sql="SELECT 
  FERPER.FOLIO_PERMISO AS FOLIO,
  FERPER.FUN_RUT AS RUT,
  UNIDAD.UNI_DESCRIPCION AS UNIDAD_PERMISO,
  FERPER.FECHA_INICIO,
  FERPER.FECHA_TERMINO,
  DATEDIFF(FERPER.FECHA_TERMINO,FERPER.FECHA_INICIO) AS DIAS,
  UCASE(TIPO_SERVICIO.TSERV_DESCRIPCION) AS TIPO_PERMISO,
...
FROM
  FERPER
  INNER JOIN UNIDAD ON (FERPER.UNI_CODIGO = UNIDAD.UNI_CODIGO)
  INNER JOIN TIPO_SERVICIO ON (FERPER.TIPO_PERMISO = TIPO_SERVICIO.TSERV_CODIGO)
  INNER JOIN FUNCIONARIO ON (FERPER.FUN_RUT = FUNCIONARIO.FUN_RUT)
  INNER JOIN GRADO ON (FUNCIONARIO.ESC_CODIGO = GRADO.ESC_CODIGO)
  AND (FUNCIONARIO.GRA_CODIGO = GRADO.GRA_CODIGO)
WHERE
  FERPER.FOLIO_PERMISO ='N24-".$_POST["texto2"]."'";
             $res=mysql_query($sql,Conectar::con());
 //echo $sql;
	// verificamos que no haya error 
	   ...
	   echo "";
        exit();
         ...
         echo "<td>N&#176;</td>";
         echo "<td>FOLIO PERMISO</td>";
         echo "<td>RUT</td>";
         echo "<td>UNIDAD ORIGEN PERMISO</td>";
         echo "<td>INICIO</td>";
         echo "<td>TERMINO</td>";
         echo "<td>DIAS</td>";
    ...
    //obtenemos los datos resultado de la consulta 
        ...
        echo "<td>".$row["RUT"]."</td>";
        echo "<td>".$row["UNIDAD_PERMISO"]."</td>";
        echo "<td>".$row["FECHA_INICIO"]."</td>";
        echo "<td>".$row["FECHA_TERMINO"]."</td>";
        echo "<td>".$row["DIAS"]."</td>";
        echo "<td>".$row["TIPO_PERMISO"]."</td>";
