...
include("../../inc/configV4.inc.php");
	...
	$sql1 = "SELECT 
			...
			WHERE CARGO_FUNCIONARIO.UNI_CODIGO = '".$unidad."' AND CARGO_FUNCIONARIO.FECHA_DESDE <= '".$fecha."' 
...
echo "<div class='linea'></div>";
	...
	$sql1 = "(SELECT
						...
						LEFT JOIN CARGO_FUNCIONARIO ON (FUNCIONARIO.FUN_CODIGO = CARGO_FUNCIONARIO.FUN_CODIGO)
						WHERE
							CARGO_FUNCIONARIO.UNI_CODIGO = '".$unidad."' 
							AND ((CARGO_FUNCIONARIO.FECHA_DESDE <= '".$fecha."' AND CARGO_FUNCIONARIO.FECHA_HASTA > '".$fecha."') 
							OR (CARGO_FUNCIONARIO.FECHA_DESDE <= '".$fecha."' AND CARGO_FUNCIONARIO.FECHA_HASTA IS NULL))
	...
	*/
	...
	while ($myrow1 = mysql_fetch_array($result1)){
		...
		$horarioAux = "";
		if($myrow1[TSERV_TIPO]=="N" && $myrow1[TSERV_CODIGO] != 142 && $myrow1[TSERV_CODIGO] != 143 && $myrow1[TSERV_CODIGO] != 144 && $myrow1[TSERV_CODIGO] != 145 && $myrow1[TSERV_CODIGO] != 146 && $myrow1[TSERV_CODIGO] != 147 && $myrow1[TSERV_CODIGO] != 148  && $myrow1[TSERV_CODIGO] != 149  && $myrow1[TSERV_CODIGO] != 151  && $myrow1[TSERV_CODIGO] != 152  && $myrow1[TSERV_CODIGO] != 153 && $myrow1[TSERV_CODIGO] != 892 && $myrow1[TSERV_CODIGO] != 870){
		    $horarioAux="";
	...
	$sql1 = "SELECT 
					...
					JOIN GRADO ON (FUNCIONARIO.ESC_CODIGO = GRADO.ESC_CODIGO) AND (FUNCIONARIO.GRA_CODIGO = GRADO.GRA_CODIGO)
					WHERE SERVICIO.UNI_CODIGO = '".$unidad."' AND SERVICIO.FECHA = '".$fecha."' AND (TIPO_SERVICIO.TSERV_TIPO <> 'N' OR (TIPO_SERVICIO.TSERV_CODIGO = 892))
					GROUP BY
					  FUNCIONARIO_SERVICIO.FUN_CODIGO,
					  FUNCIONARIO.FUN_APELLIDOPATERNO,
					  FUNCIONARIO.FUN_APELLIDOMATERNO,
  ...
  if (($tiempoPromedio%60) > 0) $horasPromedio .= " CON ".($tiempoPromedio%60)." MINUTO(S)";
	...
	$sql1 = "SELECT 
					...
					JOIN SERVICIO ON (FUNCIONARIO_SERVICIO.UNI_CODIGO = SERVICIO.UNI_CODIGO) AND (FUNCIONARIO_SERVICIO.CORRELATIVO_SERVICIO = SERVICIO.CORRELATIVO_SERVICIO)
					JOIN TIPO_SERVICIO ON (SERVICIO.TSERV_CODIGO = TIPO_SERVICIO.TSERV_CODIGO)
					WHERE SERVICIO.UNI_CODIGO = '".$unidad."' AND SERVICIO.FECHA = '".$fecha."' 
					AND TIPO_SERVICIO.TSERV_TIPO = 'N' AND SERVICIO.TSERV_CODIGO NOT IN (142, 143,144,145,146,147,148,149,151,152,153,892,870)
