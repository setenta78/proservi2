<?
	...
	$listaMediosVigilancia	= unserialize(stripslashes(utf8_decode($_POST['arrayListaMV'])));
		...
		for ($j=0;$j<count($funcionariosAsignados);$j++){
			...
			while($k<count($listaAccesorios)){
				...
				if ($accesorioFuncionario[0] == $funcionariosAsignados[$j]){
					...
					$camarasAsignados	= $accesorioFuncionario[8];
					for ($l=0; $l<count($camarasAsignados); $l++){
						$camara = new camara;
						$camara->setCodigo($camarasAsignados[$l]);
