...
function leeModeloVehiculos(marca, nombreObjeto){
	...
	objHttpXMLModelo.open("POST","./xml/xmlModeloVehiculo.php",true);
		...
		if(objHttpXMLModelo.readyState == 4){       
			if (objHttpXMLModelo.responseText != "VACIO"){
				...
				for(i=0;i<xml.getElementsByTagName('modelo').length;i++){
					codigo 			= xml.getElementsByTagName('codigoModelo')[i].text;
					descripcion 	= xml.getElementsByTagName('descripcionModelo')[i].text;
