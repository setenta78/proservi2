...
Class dbUnidad extends Conexion{	
	...
	function PrimerArbolUnidades($codPadre,$perfil,$unidades){
		...
		if($Jerarquia==10){
			...
			$filtro = " WHERE 1=1 ".$filtro;
						$sql = "SELECT 
							...
							20 AS UNI_PADRE,
							IF(UNI_TIPOUNIDAD = 150,UNI_TIPOUNIDAD,ZONA_TIPOUNIDAD) AS UNI_TIPO, IF(UNI_TIPOUNIDAD = 150,21,ZONA_ORDEN) AS ORDEN_CENCO
		...
		elseif($Jerarquia==20){
			//Zona
			$sql = "SELECT PREFECTURA_CODIGO AS UNI_CODIGO, PREFECTURA_DESCRIPCION AS UNI_DESCRIPCION, ZONA_CODIGO AS UNI_PADRE, if((PREFECTURA_TIPOUNIDAD = 30 OR PREFECTURA_TIPOUNIDAD = 120),PREFECTURA_TIPOUNIDAD,0) AS UNI_TIPO, PREFECTURA_ESPECIALIDAD AS UNI_ESPECIALIDAD 
				FROM VISTA_ARBOL_UNIDADES_NACIONAL
		...
		elseif($Jerarquia==30 || $Jerarquia==40 || $Jerarquia==120 || $Jerarquia==180){
			//Prefectura
			$sql = "SELECT COMISARIA_CODIGO AS UNI_CODIGO, COMISARIA_DESCRIPCION AS UNI_DESCRIPCION, PREFECTURA_CODIGO AS UNI_PADRE, if((COMISARIA_TIPOUNIDAD = UNI_TIPOUNIDAD),COMISARIA_TIPOUNIDAD,0) AS UNI_TIPO, UNI_ESPECIALIDAD
				FROM VISTA_ARBOL_UNIDADES_NACIONAL
				WHERE PREFECTURA_CODIGO = ".$codPadre." ".$filtro."
	...
	function ArbolUnidades($codPadre,$jerarquia,$perfil,$unidades){
		...
		if($jerarquia==1){
			...
			//Prefectura
			$sql = "SELECT COMISARIA_CODIGO AS UNI_CODIGO, COMISARIA_DESCRIPCION AS UNI_DESCRIPCION, DEPENDIENTE_CODIGO AS UNI_PADRE, if((COMISARIA_TIPOUNIDAD = UNI_TIPOUNIDAD),COMISARIA_TIPOUNIDAD,0) AS UNI_TIPO, UNI_ESPECIALIDAD
				FROM VISTA_ARBOL_UNIDADES_NACIONAL
