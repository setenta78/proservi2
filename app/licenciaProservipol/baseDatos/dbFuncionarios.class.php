...
{			
	//Funcion modificada el 28-04-2015
    //Campos agregados a la consulta:
	...
	function listaTotalFuncionarios($Unidad, $nombreBucar, $escalafon, $grado, $NombreCampo, $TipoOrden, $funcionarios){
		...
		$sql .= " ORDER BY ".$campoOrdenar;
				...
				while($myrow = mysql_fetch_array($result)){
                    ...
                    $persona->setSeccion($seccion); //Campo agregado el 28-04-2015
	...
	//FIN FUNCION
     ...
     function buscarFuncionarioPersonal($codigoFuncionario, $funcionarios){    	    
	    ...
	    $sql = "SELECT 
								...
								pesbasi.PEFBGRA
							FROM pesbasi
							WHERE pesbasi.PEFBCOD = '".$codigoFuncionario."'";
	    	//echo $sql;
		...
		//Replica en otra BD
            function updateCargoFuncionarioPaso($funcionario, $fechaNuevoCargo){
			
			$sql = "UPDATE CARGO_FUNCIONARIO_PASO SET
					FECHA_HASTA = '".$fechaNuevoCargo."'
					WHERE FUN_CODIGO = '".$funcionario->getCodigoFuncionario()."' AND FECHA_HASTA IS NULL";
		...
		function nuevoFuncionario($funcionario){ 
			...
			$sql = "INSERT INTO FUNCIONARIO 
				   (FUN_CODIGO, ESC_CODIGO, GRA_CODIGO, FUN_APELLIDOPATERNO, FUN_APELLIDOMATERNO, FUN_NOMBRE, UNI_CODIGO) VALUES
		...
		function listaFuncionariosDisponibles($unidad, $fechaServicio, $tipoServicio, $correlativo, $servicio, $funcionarios){
			...
			$sql = "(SELECT
				...
				//Control modificaddo agregando los codigos; 629,630,631
                //Fecha: 30-04-2014
				if ($servicio == 607){
					   $sql .= " AND FUNCIONARIO.FUN_CODIGO IN (
									SELECT                                                                           
						        	...
						        	FROM                                                                             
						          		...
						          		INNER JOIN SERVICIO ON (FUNCIONARIO_SERVICIO.UNI_CODIGO = SERVICIO.UNI_CODIGO) 
						          		AND (FUNCIONARIO_SERVICIO.CORRELATIVO_SERVICIO = SERVICIO.CORRELATIVO_SERVICIO)
						        	WHERE SERVICIO.FECHA = '".$fechaServicio."' AND SERVICIO.TSERV_CODIGO IN (191, 194, 195, 196, 197, 198, 625, 635, 636, 665, 666, 667))";
				}
						...
						|| $servicio != 120 || $servicio != 130 || $servicio != 140 || $servicio != 141 || $servicio != 150 || $servicio != 160 || $servicio != 606 || $servicio != 634 || $servicio != 713
						|| $servicio != 718 || $servicio != 719 || $servicio != 720 || $servicio != 721){
				$sql .= " AND FUNCIONARIO.FUN_CODIGO NOT IN (
						SELECT                                                                           
			          		FUNCIONARIO_SERVICIO.FUN_CODIGO                                                
			        	FROM                                                                             
			          		...
			          		INNER JOIN SERVICIO ON (FUNCIONARIO_SERVICIO.UNI_CODIGO = SERVICIO.UNI_CODIGO) 
					        	...
					        	WHERE SERVICIO.FECHA = '".$fechaServicio."' AND TIPO_SERVICIO.TSERV_CODIGO IN(120,130,140,141,150,160,161,162,170,180,606,633,634,713,718,719,720,721))";
				...
				$sql .= " AND FUNCIONARIO.FUN_CODIGO NOT IN (
			        	...
			        	FROM                                                                             
			          		...
			          		INNER JOIN SERVICIO ON (FUNCIONARIO_SERVICIO.UNI_CODIGO = SERVICIO.UNI_CODIGO) 
						          		AND (FUNCIONARIO_SERVICIO.CORRELATIVO_SERVICIO = SERVICIO.CORRELATIVO_SERVICIO)
                                 INNER JOIN TIPO_SERVICIO ON (SERVICIO.TSERV_CODIGO = TIPO_SERVICIO.TSERV_CODIGO)
					        	WHERE SERVICIO.FECHA = '".$fechaServicio."' AND TIPO_SERVICIO.TSERV_CODIGO IN(120,130,140,141,150,160,161,162,170,180,606,63,634,713,718,719,720,721))";
