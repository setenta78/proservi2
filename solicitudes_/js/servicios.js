		...
		{       
			...
			if (objHttpXMLServicios.responseText != "VACIO"){
				...
				for(i=0;i<xml.getElementsByTagName('servicio').length;i++){
					...
					if (fechaValidacion != "" || (unidadBloqueda == 1 && comparaFechaLimite == 2)) var dblClick = "javascript:abrirVentana('DETALLE SERVICIO ...', '995', '510','fichaServicio.php?unidad="+unidad+"&correlativo="+correlativo+"', '','','0','0')";
...
function vistaMediosDeVigilancia(mediosDeVigilancia){
	...
	for(var i=0;i<mediosDeVigilancia.getElementsByTagName('medioVigilancia').length;i++){
		...
		//var codigoCaballo 	= vehiculo.getElementsByTagName('codigoCaballo')[0].text;
		//var nombreCaballo	= vehiculo.getElementsByTagName('nombreCaballo')[0].text;
		//var tipoAnimal	 = vehiculo.getElementsByTagName('tipoAnimal')[0].text;
				
		//if (codigoVehiculo==0) var descripcionVehiculo	= tipoAnimal + " (" + nombreCaballo + ")";
		//else var descripcionVehiculo	= tipoVehiculo + " (" + patenteVehiculo + ")";
...
function leeAnimalesServicio(vehiculo){

	//alert(vehiculo);
	var codigoCaballo 	= vehiculo.getElementsByTagName('codigoCaballo')[i].text;
	var nombreCaballo	= vehiculo.getElementsByTagName('nombreCaballo')[i].text;
	var tipoAnimal	= vehiculo.getElementsByTagName('tipoAnimal')[i].text;
  //alert(codigoCaballo);
				
	var descripcion		= tipoAnimal + " (" + nombreCaballo + ")";
	
	
	if (codigoCaballo != 0){	
		var datosOpcion = new Option(descripcion, codigoCaballo, "", "");
		document.getElementById('caballosAsignados').options[document.getElementById('caballosAsignados').length] = datosOpcion;
	}
	
	//document.getElementById('factorMV').value = factor; 
}

function leeSimccarServicio(vehiculo){

	//alert(vehiculo);
	var codigoSimccar 	= vehiculo.getElementsByTagName('codigoSimccar')[i].text;
	var serieSimccar  = vehiculo.getElementsByTagName('numeroSerie')[i].text;
  //alert(codigoSimccar);
				
	var simccar="SIMCCAR";
	var descripcion		= simccar + " (" + serieSimccar + ")";
...
function llenaArregloMediosDeVigilancia(mediosDeVigilancia){
	...
	for(var i=0;i<mediosDeVigilancia.getElementsByTagName('medioVigilancia').length;i++){
		...
		//var codigoSimccar 	= vehiculo.getElementsByTagName('codigoSimccar')[0].text;
		//var serieSimccar	 = vehiculo.getElementsByTagName('serieSimccar')[0].text;
		...
		var descCaballo		  = tipoAnimal + " (" + nombreCaballo + ")";
		//alert(descCaballo);
		document.getElementById('caballosServicio').length = null;
		var datosOpcion2 = new Option(descCaballo, codigoCaballo, "", "");
		document.getElementById('caballosServicio').options[document.getElementById('caballosServicio').length] = datosOpcion2;
		
		//var simccar="SIMCCAR";
		//var descSimccar	  = simccar + " (" + serieSimccar + ")";
		//alert(descCaballo);
		//document.getElementById('simccarServicio').length = null;
		//var datosOpcion3 = new Option(descSimccar, codigoSimccar, "", "");
		//document.getElementById('simccarServicio').options[document.getElementById('simccarServicio').length] = datosOpcion3;
		
		//alert(document.getElementById('caballosServicio').length);
...
function llenaArregloFuncionariosAccesorios(mediosDeVigilancia){
	for(var i=0;i<mediosDeVigilancia.getElementsByTagName('funcionario').length;i++){
		...
		//OBTIENE LOS DATOS DE LAS SIMCCAR UTILIZADAS POR EL FUNCIONARIO;
		var simccars = funcionario.getElementsByTagName('simccars')[0];
		//alert();
		for (var k=0; k<simccars.getElementsByTagName('simccar').length; k++){
			//alert();
			var simccar			= simccars.getElementsByTagName('simccar')[k];
			var codigoSimccar 		= "S" + simccar.getElementsByTagName('codigoSimccar')[0].text;
			var numeroSerie 	= simccar.getElementsByTagName('numeroSerie')[0].text;
						
			var descripcionSimccar =  "SIMCCAR ("+numeroSerie + ")";
			
			var datosOpcion = new Option(descripcionSimccar, codigoSimccar, "", "");
			document.getElementById('personalServicioAccesorio').options[document.getElementById('personalServicioAccesorio').length] = datosOpcion;   
			
			//var asignadosSimccar = document.getElementById('simccarAsignados').value;
			//alert(asignadosSimccar);
			//if(asignadosSimccar!=""){
			//var datosOpcion2 = new Option(asignadosSimccar, codigoSimccar, "", "");
			//document.getElementById('simccarAsignados').options[document.getElementById('simccarAsignados').length] = datosOpcion2;
			//alert();
		 //}
		}
...
function buscaColacionPorFuncionario(unidadServicio, fecha1, fecha2){
	
	var objHttpXMLServiciosPorFuncionario = new AJAXCrearObjeto();
    objHttpXMLServiciosPorFuncionario.open("POST","./xml/xmlServicios/xmlListaColacionesPorFuncionarios.php",false);
	objHttpXMLServiciosPorFuncionario.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	objHttpXMLServiciosPorFuncionario.send(encodeURI("unidadServicio="+unidadServicio+"&fecha1="+fecha1+"&fecha2="+fecha2));  
	//alert(objHttpXMLServiciosPorFuncionario.responseText);	   
